...
class Block(object):
    ...
    def __str__(self):
        return "abs_pos ", self.abs_pos, " rel_pos ", self.rel_pos
...
class BlockTracker(object):
    ...
    def detect(self, img):        
        ...
        cv2.imshow("block_draw", blocks_img)
...
#        self.display_img = display_img_out
    ...
    def _detect_blocks(self, blocks_img):
        cv2.imshow("lap_Img", blocks_img)
        
        lap_img = cv2.Laplacian(blocks_img, cv2.CV_8U)
        cv2.imshow("lap_img", lap_img)
        
        _, contours, _ = cv2.findContours(lap_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
